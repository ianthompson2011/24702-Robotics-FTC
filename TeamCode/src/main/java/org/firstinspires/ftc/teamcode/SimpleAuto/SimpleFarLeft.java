...
@Autonomous(name = "SimpleFarLeft")
public class SimpleFarLeft extends OpMode {
    ...
    private final Pose parkAfterShoot   = new Pose(56,  122, Math.toRadians(90));  // used after shooting
    private final Pose parkDirectPose   = new Pose(108, 9,   Math.toRadians(90));  // park-only destination
    ...
    public void stateUpdate() {
        switch (pathState) {
            ...
            case PARK:
                if (!pathStarted) {
                    follower.setMaxPower(FULL_POWER);
                    // shoot mode parks from shootPose; park-only parks from startPose
